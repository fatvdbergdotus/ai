...
- **Data augmentation** is a key technique to improve models by creating more training data from existing data.
...
## E
- **Encoding techniques of features** [Which models and encoding techniques to combine](models_enoding_techniques.md)
  ...
  - **Binary encoding**, or Base-2 encoding, is a technique that converts categorical variables into binary (0/1) representations using fewer columns than one-hot encoding. Each category is first assigned a number, which is then converted into binary form and split across multiple columns. This reduces dimensionality while still representing categorical information, making it more efficient for models with many categories.
  - **Dummy encoding** is like one-hot encoding. However, if you include all columns, they become redundant (perfect multicollinearity), so one is dropped instead. Perfect multicollinearity is a modeling issue where two or more independent variables in a regression model have an exact linear relationship, meaning one can be predicted perfectly from the others. 
...
- **Stratified sampling** is a technique where you split your dataset into groups (strata) based on a characteristic (often the target variable), and then sample proportionally from each group.
## T
